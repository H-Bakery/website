...
describe('RecipeList Component', () => {
  ...
  test('renders correctly with a list of recipes', () => {
    ...
    expect(
      screen.getByText(mockRecipes[0].prepTime as string)
    ).toBeInTheDocument()
    ...
    expect(
      screen.getByText(mockRecipes[1].prepTime as string)
    ).toBeInTheDocument()
  })
  ...
  test('displays a message when the recipe list is empty', () => {
    ...
    expect(
      screen.getByText('Try adding a new recipe to get started!')
    ).toBeInTheDocument()
  })
  ...
  test('calls onSelectRecipe with the correct recipe when "View" button is clicked', () => {
    ...
    const viewButton = within(firstRecipeRow).getByRole('button', {
      name: /view/i,
    })
  ...
  test('calls onEditRecipe with the correct recipe when "Edit" button is clicked', () => {
    ...
    const editButton = within(secondRecipeRow).getByRole('button', {
      name: /edit/i,
    })
