...
const ContentPreview: React.FC<ContentPreviewProps> = ({
  ...
  const getTextContent = (template: Template) => {
    ...
    if (
      ...
      template.textElements.some((el) => el.id === 'variant')
    ) {
            ...
            <Box
              ...
              sx={{
                ...
                transform: `scale(${Math.min(
                  280 / (template.width || 1080),
                  280 / (template.height || 1080)
                )})`,
                position: 'absolute',
                top: 0,
                left: 0,
            ...
            >
              ...
              >
                {templateType === 'message' ? (
                  ...
                  <Typography
                    style={getSimpleSquareTextStyles(
                      content.title || getPlaceholderText(templateType, 'title')
                    )}
                    sx={{
                      color:
                        template.textPanelStyle?.textColor ||
                        template.colors.primary,
                      fontWeight: 'bold',
                      fontFamily: templateConfig.typography.title.fontFamily,
                      letterSpacing: 0.5,
                  ...
                  <>
                    ...
                    >
                      {content.title ||
                        getPlaceholderText(templateType, 'title')}
                    </Typography>
                    ...
                    {shouldShowDescription(
                      ...
                      <Typography
                        sx={{
                          ...
                          fontWeight: 'normal',
                          fontSize:
                            templateConfig.typography.description.fontSize,
                          mb: 2,
                          fontFamily:
                            templateConfig.typography.description.fontFamily,
                          letterSpacing: 0.2,
                          lineHeight: 1.4,
                        }}
                      >
                        {content.description ||
                          getPlaceholderText(templateType, 'description')}
                      </Typography>
                    )}
                    ...
                    {(content.price || shouldShowPrice(templateType)) && (
                      <Typography
                        sx={{
                          ...
                          mb: 1,
                          fontFamily:
                            templateConfig.typography.price.fontFamily,
                        }}
                      >
                        {content.price
                          ? `${content.price} €`
                          : getPlaceholderText(templateType, 'price')}
                      </Typography>
                    )}
                    ...
                    {content.additionalInfo && (
                      <Typography
                        sx={{
                          color: template.textPanelStyle?.textColor || 'white',
                          fontSize:
                            templateConfig.typography.additionalInfo.fontSize,
                          fontFamily:
                            templateConfig.typography.additionalInfo.fontFamily,
                          opacity: 0.9,
                          mt: 1,
                        }}
                      >
