...
const TestComponent = () => {
  ...
  return (
    <div>
      ...
      <button
        onClick={() => addToCart(mockProducts[1])}
      ...
      <button
        onClick={() =>
          updateQuantity(
            1,
            items.find((item) => item.id === 1)?.quantity
              ? items.find((item) => item.id === 1)!.quantity - 1
              : 0
          )
        }
        data-testid="decrease-product-1"
      >
        Decrease Bread
      </button>
      <button onClick={() => updateQuantity(1, 5)} data-testid="set-quantity-5">
        Set Bread to 5
      </button>
...
describe('CartContext', () => {
  ...
  it('adds multiple different products to the cart', () => {
    ...
    expect(screen.getByTestId('cart-items')).toHaveTextContent(
      'Sourdough Bread:1,Chocolate Croissant:1'
    )
  })
  ...
  it('calculates totals correctly with multiple items', () => {
    ...
    expect(screen.getByTestId('cart-items')).toHaveTextContent(
      'Sourdough Bread:3,Chocolate Croissant:2'
    )
  })
...
const TestNegativeQuantityComponent = () => {
  const { items, totalPrice, totalCount, addToCart, updateQuantity } =
    React.useContext(CartContext)
