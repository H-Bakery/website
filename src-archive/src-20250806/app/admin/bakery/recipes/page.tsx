...
} from '../../../../services/types' // Added ReviewType for handlers
...
export default function RecipeManagementPage() {
  ...
  const handleAddRecipeSubmit = async (
    ...
    newRecipeData: Omit<
      RecipeType,
      'id' | 'reviews' | 'slug' | 'instructionsHtml' | 'createdAt' | 'updatedAt'
    > & { category: string; prepTime: string }
  ) => {
    try {
      setError(null)
    ...
    return (
      <RecipeDetailView
        ...
        onAddReview={(reviewData) =>
          handleAddReview(String(viewingRecipe.id), reviewData)
        }
        onUpdateReview={(reviewData) =>
          handleUpdateReview(String(viewingRecipe.id), reviewData)
        }
        onDeleteReview={(reviewId) =>
          handleDeleteReview(String(viewingRecipe.id), reviewId)
        }
      />
    )
  }
  ...
  if (showRecipeForm) {
    return (
      <RecipeForm
        ...
        onSubmit={
          editingRecipe ? handleUpdateRecipeSubmit : handleAddRecipeSubmit
        }
  ...
  return (
    <Container maxWidth="lg" sx={{ py: { xs: 2, md: 4 } }}>
      <Paper
        elevation={0}
        sx={{ p: { xs: 2, md: 3 }, backgroundColor: 'transparent' }}
      >
        <Box
          sx={{
            display: 'flex',
