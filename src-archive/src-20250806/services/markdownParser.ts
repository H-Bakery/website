...
export class MarkdownParser {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('## ') && line.includes('Pastries Preparation')) {
        ...
        currentSection = {
          ...
          instructions: [
            'All items must be placed on trays with protective foil',
          ],
          items: [],
          completed: false,
        }
        ...
        line.startsWith('## ') &&
        line.includes('Sourdough Preparation')
      ) {
        if (currentSection) sections.push(currentSection as PrepSection)
        currentSection = {
        ...
        line.startsWith('## ') &&
        line.includes('Brühstück Preparation')
      ) {
        if (currentSection) sections.push(currentSection as PrepSection)
        currentSection = {
        ...
        line.startsWith('## ') &&
        line.includes('Meatloaf Preparation')
      ) {
        if (currentSection) sections.push(currentSection as PrepSection)
        currentSection = {
      ...
      if (isInTable && line.startsWith('|') && currentMode === 'pastry_table') {
        ...
        if (cells.length >= 3) {
          ...
          if (trayInfo.includes('-')) {
            // Range like "11-13"
            const [start, end] = trayInfo
              .split('-')
              .map((n) => parseInt(n.trim()))
            item.tray_numbers = Array.from(
              { length: end - start + 1 },
              (_, i) => start + i
            )
  ...
  static parseBakingSchedule(content: string): {
    cakes: BakingItem[]
    bread: BakingItem[]
  } {
    const lines = content.split('\n')
  ...
  private static addStockStatus(item: PrepTaskItem): void {
    ...
    const statuses: Array<'sufficient' | 'low' | 'critical' | 'empty'> = [
      'sufficient',
      'low',
      'critical',
      'empty',
