...
class PrepTaskService {
  ...
  async loadPrepTasks(date: Date): Promise<PrepTaskResponse> {
    try {
      ...
      console.error(
        'Error loading prep tasks, using default configuration:',
        error
      )
  ...
  async getPrepTaskHistory(days: number = 7): Promise<
    Array<{
      date: string
      completed: boolean
  ...
  > {
    try {
      const response = await fetch(
        `${API_BASE_URL}/prep-tasks/history?days=${days}`
      )
    ...
    return {
      sections: [
        {
          ...
          description:
            'Artikel vom Gefrierschrank auf den Kaffeestückchen Wagen räumen',
          ...
          items: [
            ...
            {
              name: 'Pudding Schnecke',
              quantity: 6,
              tray_number: 5,
              completed: false,
            },
            {
              name: 'Pudding Plunder',
              quantity: 7,
              tray_number: 6,
              completed: false,
            },
      ...
      bakingSchedule: {
        cakes: [
          ...
          {
            name: 'Käsekuchen mit Mandarinen',
            standard_quantity: 2,
            quantity: 2,
          },
          {
  ...
  private getMockHistory(days: number): Array<{
    ...
    for (let i = 0; i < days; i++) {
      ...
      const completionRate = completed
        ? Math.random() * 20 + 80
        : Math.random() * 60 // 80-100% if completed, 0-60% if not
      ...
      history.push({
        ...
        completedAt: completed
          ? `${14 + Math.floor(Math.random() * 2)}:${Math.floor(
              Math.random() * 60
            )
              .toString()
              .padStart(2, '0')}`
