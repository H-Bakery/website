    ...
    <Paper sx={{ p: 3 }} className="production-checklist">
      ...
      <Box mb={4}>
        <Typography variant="h5" component="h3" gutterBottom>
          1. Hefeteig Vorbereitung ({productionPlan.doughBatches} Chargen à
          40kg)
        </Typography>
        <Divider sx={{ mb: 2 }} />
        ...
        <Grid container spacing={2}>
          {Array(productionPlan.doughBatches)
            ...
            .map((_, index) => {
              ...
              const batchWeight =
                isLastBatch && productionPlan.totalDoughWeight % 40000 !== 0
                  ? ((productionPlan.totalDoughWeight % 40000) / 1000).toFixed(
                      1
                    )
                  : 40
                ...
                if (amount > 0) {
                  return (
                    <Grid item xs={12} key={`filling-${filling}`}>
                      <FormControlLabel
                        control={<Checkbox />}
                        label={`${formatFillingName(filling)}: ${(
                          amount / 1000
                        ).toFixed(1)} kg`}
                      />
                    </Grid>
              ...
              return (
                <Grid item xs={12} sm={6} md={4} key={`product-${product}`}>
                  ...
                  {calculator.recipes[product]?.doughPiece && (
                    ...
                    >
                      aus{' '}
                      {calculator.getDoughPieceDetails(
                        calculator.recipes[product].doughPiece
                      )?.count || 0}{' '}
                      Teigstücken geflochten
                    </Typography>
                  )}
                </Grid>
              )
