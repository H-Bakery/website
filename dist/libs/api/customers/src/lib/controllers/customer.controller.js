      ...
      return tslib_1.__generator(this, function (_b) {
        switch (_b.label) {
          ...
          case 3:
            newUser = _b.sent()
            core_1.logger.info(
              'User created successfully with ID: '.concat(newUser.id)
            )
            res.json({
      ...
      return tslib_1.__generator(this, function (_b) {
        switch (_b.label) {
          ...
          case 2:
            ...
            if (!user) {
              core_1.logger.info(
                'Login failed: User '.concat(username, ' not found')
              )
              res.status(400).json({ error: 'Invalid credentials' })
              return [2 /*return*/]
            }
            core_1.logger.info(
              'User found with ID: '.concat(user.id, ', validating password...')
            )
          ...
          case 3:
            ...
            if (!validPassword) {
              core_1.logger.info(
                'Login failed: Invalid password for user '.concat(username)
              )
              res.status(400).json({ error: 'Invalid credentials' })
              return [2 /*return*/]
            }
            core_1.logger.info(
              'Password valid, generating token for user '.concat(username)
            )
      ...
      return tslib_1.__generator(this, function (_a) {
        switch (_a.label) {
          case 0:
            ...
            return [
              4 /*yield*/,
              customer_model_1.Customer.findByPk(customerId, {
                attributes: { exclude: ['password'] },
              }),
            ]
          case 1:
      ...
      return tslib_1.__generator(this, function (_b) {
        switch (_b.label) {
          case 0:
            ...
            core_1.logger.info(
              'Updating password for customer: '.concat(customerId)
            )
            return [4 /*yield*/, customer_model_1.Customer.findByPk(customerId)]
          case 1:
            customer = _b.sent()
