...
import { createWhatsAppLink, contactConfig } from '@bakery/shared/utils'
...
const QuickOrder: React.FC = () => {
  ...
  const filteredItems = quickOrderItems.filter(
    (item) =>
  ...
  const totalPrice = Object.entries(orderItems).reduce((sum, [itemId, qty]) => {
  ...
  const handleOrder = () => {
    ...
    const message = `Hallo! Ich möchte folgendes bestellen: ${orderText}. Gesamtpreis: €${totalPrice.toFixed(
      2
    )}`
              ...
              <Fade in={true} timeout={500 + index * 100}>
                <Paper
                  ...
                  sx={{
                    ...
                    borderRadius: 2,
                    border: '2px solid',
                    borderColor: orderItems[item.id]
                      ? 'primary.main'
                      : 'transparent',
