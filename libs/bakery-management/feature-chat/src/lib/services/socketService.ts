...
type NotificationUpdateCallback = (update: {
  id: number
  [key: string]: any
}) => void
...
class SocketService {
  ...
  private setupEventHandlers() {
    ...
    this.socket.on('notification:new', (notification: Notification) => {
      this.listeners.onNewNotification.forEach((callback) =>
    ...
    this.socket.on(
      ...
      (update: { id: number; [key: string]: any }) => {
        this.listeners.onNotificationUpdate.forEach((callback) =>
          callback(update)
        )
      }
    )
  ...
  onNewNotification(callback: NotificationCallback) {
    ...
    return () => {
      this.listeners.onNewNotification =
        this.listeners.onNewNotification.filter((cb) => cb !== callback)
    }
  }
  ...
  onNotificationUpdate(callback: NotificationUpdateCallback) {
    ...
    return () => {
      this.listeners.onNotificationUpdate =
        this.listeners.onNotificationUpdate.filter((cb) => cb !== callback)
    }
  }
  ...
  onNotificationDelete(callback: NotificationDeleteCallback) {
    ...
    return () => {
      this.listeners.onNotificationDelete =
        this.listeners.onNotificationDelete.filter((cb) => cb !== callback)
    }
  }
