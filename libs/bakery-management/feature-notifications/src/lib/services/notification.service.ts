import { Notification, NotificationFilters, NotificationStats } from '@bakery/shared/types';
...
class NotificationService {
  ...
  async getNotifications(filters?: NotificationFilters): Promise<Notification[]> {
    ...
    try {
      ...
      if (filters?.priorities?.length) {
        filters.priorities.forEach(prio => params.append('priority', prio));
      }
  ...
  private markAsReadMock(id: string): Notification | null {
  ...
  private deleteNotificationMock(id: string): boolean {
    const index = this.notifications.findIndex(n => n.id === id);
    if (index !== -1) {
      this.notifications.splice(index, 1);
      this.notifyListeners();
