...
describe('InventoryForm', () => {
  ...
  describe('Create Mode', () => {
    ...
    it('should show validation errors for required fields', async () => {
      ...
      // Speichern ist erst aktiv, wenn das Formular geändert wurde
      const user = userEvent.setup()
      await user.type(screen.getByLabelText(/lagerort/i), 'A1')
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/produkt ist erforderlich/i)
        ).toBeInTheDocument()
      })
    })
  })
  ...
  describe('Edit Mode', () => {
    ...
    it('should render edit form with existing data', () => {
      render(<InventoryForm {...defaultProps} mode="edit" item={mockItem} />)
  ...
  describe('Form Interactions', () => {
    ...
    it('should display error message when provided', () => {
      ...
      render(<InventoryForm {...defaultProps} error={errorMessage} />)

      expect(screen.getByText(errorMessage)).toBeInTheDocument()
    })

    it('should disable submit button when loading', () => {
      render(<InventoryForm {...defaultProps} loading={true} />)
  ...
  describe('Field Validation', () => {
    it('should validate minimum quantity', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/mindestbestand muss mindestens 0 sein/i)
        ).toBeInTheDocument()
      })
    })
    ...
    it('should validate reorder point', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/bestellpunkt muss mindestens 0 sein/i)
        ).toBeInTheDocument()
      })
    })
  })
