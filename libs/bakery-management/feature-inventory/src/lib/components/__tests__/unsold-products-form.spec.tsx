...
const mockProducts = [
  ...
  { id: 2, name: 'Croissant', category: 'Gebäck', price: 1.8 },
  { id: 3, name: 'Apfelkuchen', category: 'Kuchen', price: 2.5 },
]
...
describe('UnsoldProductsForm', () => {
  ...
  it('renders form elements correctly', async () => {
    ...
    expect(
      screen.getByText('Unverkaufte Produkte erfassen')
    ).toBeInTheDocument()
    ...
    expect(
      screen.getByRole('button', { name: /unverkaufte produkte speichern/i })
    ).toBeInTheDocument()
  ...
  it('disables submit button when form is incomplete', async () => {
    ...
    const submitButton = screen.getByRole('button', {
      name: /unverkaufte produkte speichern/i,
    })
  ...
  it('displays error when product loading fails', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Fehler beim Laden der Produkte')
      ).toBeInTheDocument()
    })
  })
