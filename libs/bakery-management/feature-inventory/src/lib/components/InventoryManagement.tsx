...
export const InventoryManagement: React.FC = () => {
  ...
  // Queries
  const { data: inventoryData, isLoading, error } = useInventory(filters, pagination);
  ...
  const handleDelete = async (item: InventoryItem) => {
    if (confirm(`Möchten Sie den Lagerbestand für "${item.product?.name}" wirklich löschen?`)) {
      try {
  ...
  const handleFormSubmit = async (data: CreateInventoryDto | UpdateInventoryDto) => {
    try {
      if (formMode === 'create') {
        await createMutation.mutateAsync(data as CreateInventoryDto);
        showSnackbar('Lagerbestand erfolgreich erstellt', 'success');
      } else if (selectedItem) {
        await updateMutation.mutateAsync({ id: selectedItem.id, data: data as UpdateInventoryDto });
  ...
  if (error) {
    return (
      <Alert severity="error">
        Fehler beim Laden der Inventardaten. Bitte versuchen Sie es später erneut.
      </Alert>
    );
  }
          ...
          <Paper sx={{ p: 3, textAlign: 'center' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
              ...
              <Typography variant="h3" color="error.main">
                {lowStockItems?.filter(item => item.quantity <= item.minimumQuantity).length || 0}
              </Typography>
            </Box>
