...
export class PrepTaskLoader {
  ...
  private static async loadConfig(): Promise<PrepConfig> {
    ...
    this.config = {
      pastry_cart: {
        description:
          'Items to be removed from freezer and arranged on the Kaffeestückchen Wagen cart',
      ...
      standard_baking: {
        cakes: [
          ...
          {
            name: 'Kirsch-Streuselkuchen',
            standard_quantity: 2,
            note: 'für nächsten Tag, 06:30',
          },
        ],
        bread: [
  ...
  private static applyStockStatus(items: PrepTaskItem[]): PrepTaskItem[] {
    ...
    return items.map((item) => {
      const stockStatuses: Array<'sufficient' | 'low' | 'critical' | 'empty'> =
        ['sufficient', 'low', 'critical', 'empty']
  ...
  public static async generatePrepTasksForDate(
    prepDate: Date
  ): Promise<PrepSection[]> {
    ...
    // 1. Pastry Cart Setup
    const pastryItems: PrepTaskItem[] = config.pastry_cart.items.map(
      (item) => ({
        name: item.name,
  ...
  public static async getBakingScheduleForDate(
    prepDate: Date
  ): Promise<{ cakes: BakingItem[]; bread: BakingItem[] }> {
    const config = await this.loadConfig()
  ...
  private static getDayVariations(dayOfWeek: number): {
    pastry: number
    cakes: number
    bread: number
  } {
  ...
  public static async loadSpecificDate(
    dateString: string
  ): Promise<PrepSection[] | null> {
