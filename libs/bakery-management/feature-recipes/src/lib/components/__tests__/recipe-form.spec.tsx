...
describe('RecipeForm Component', () => {
  ...
  describe('Add Mode', () => {
    ...
    test('allows input into fields', () => {
      ...
      fireEvent.change(screen.getByLabelText(/description/i), {
        target: { value: 'A tasty cake' },
      })
      expect(screen.getByLabelText(/description/i)).toHaveValue('A tasty cake')

      fireEvent.change(screen.getAllByLabelText(/ingredient name/i)[0], {
        target: { value: 'Sugar' },
      })
      expect(screen.getAllByLabelText(/ingredient name/i)[0]).toHaveValue(
        'Sugar'
      )

      fireEvent.change(screen.getAllByLabelText(/quantity/i)[0], {
        target: { value: '1 cup' },
      })
      expect(screen.getAllByLabelText(/quantity/i)[0]).toHaveValue('1 cup')

      fireEvent.change(screen.getByLabelText(/instructions/i), {
        target: { value: 'Mix well\\nBake at 180C' },
      })
      expect(screen.getByLabelText(/instructions/i)).toHaveValue(
        'Mix well\\nBake at 180C'
      )
    ...
    test('adds and removes ingredient fields', () => {
      ...
      fireEvent.change(screen.getAllByLabelText(/ingredient name/i)[1], {
        target: { value: 'Eggs' },
      })
      expect(screen.getAllByLabelText(/ingredient name/i)[1]).toHaveValue(
        'Eggs'
      )
      ...
      if (!lastIngredientRow)
        throw new Error('Could not find the last ingredient row')

      const removeButtonInLastRow =
        within(lastIngredientRow).getByRole('button')
    ...
    test('submits correct data for a new recipe', () => {
      ...
      fireEvent.change(screen.getByLabelText(/preparation time/i), {
        target: { value: '45 mins' },
      })
      fireEvent.change(screen.getByLabelText(/description/i), {
        target: { value: 'Classic Italian dish' },
      })
      fireEvent.change(screen.getAllByLabelText(/ingredient name/i)[0], {
        target: { value: 'Minced Beef' },
      })
      fireEvent.change(screen.getAllByLabelText(/quantity/i)[0], {
        target: { value: '500g' },
      })
      fireEvent.change(screen.getByLabelText(/instructions/i), {
        target: { value: 'Sauté onions\\nAdd beef\\nSimmer with tomatoes' },
      })
  ...
  describe('Edit Mode', () => {
    ...
    test('renders correctly with pre-filled fields', () => {
      expect(
        screen.getByText(`Edit Recipe: ${mockRecipeToEdit.name}`)
      ).toBeInTheDocument()
      expect(screen.getByLabelText(/recipe name/i)).toHaveValue(
        mockRecipeToEdit.name
      )
      expect(screen.getByLabelText(/category/i)).toHaveValue(
        mockRecipeToEdit.category
      )
      expect(screen.getByLabelText(/preparation time/i)).toHaveValue(
        mockRecipeToEdit.prepTime
      )
      expect(screen.getByLabelText(/description/i)).toHaveValue(
        mockRecipeToEdit.description
      )

      expect(screen.getAllByLabelText(/ingredient name/i)[0]).toHaveValue(
        mockRecipeToEdit.ingredients[0].name
      )
      expect(screen.getAllByLabelText(/quantity/i)[0]).toHaveValue(
        mockRecipeToEdit.ingredients[0].quantity
      )
      expect(screen.getAllByLabelText(/ingredient name/i)[1]).toHaveValue(
        mockRecipeToEdit.ingredients[1].name
      )
      expect(screen.getAllByLabelText(/quantity/i)[1]).toHaveValue(
        mockRecipeToEdit.ingredients[1].quantity
      )
    ...
    test('submits correct data for an existing recipe (maintaining ID, etc.)', () => {
      ...
      fireEvent.change(screen.getByLabelText(/category/i), {
        target: { value: 'Quick Meals' },
      })
      // User changes one ingredient
      fireEvent.change(screen.getAllByLabelText(/quantity/i)[0], {
        target: { value: '1.5 cups' },
      })
