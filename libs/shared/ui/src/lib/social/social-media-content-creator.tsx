...
const SocialMediaContentCreator: React.FC = () => {
  ...
  const handleTemplateTypeChange = (type: TemplateType) => {
    ...
    setContent((prev) => ({
      ...
      title: '',
      price:
        type === 'bakery-news' || type === 'message' || type === 'simple-square'
          ? ''
          : prev.price,
      // Clear description for message and simple-square types since they're not used
      description:
        type === 'message' || type === 'simple-square' ? '' : prev.description,
    }))
    setIsComplete(false)
  }
    ...
    try {
      ...
      const mappedContent = {
        textElements: {
          ...
          subtitle:
            templateType === 'offer'
              ? content.additionalInfo || 'Für kurze Zeit'
              : '',
          // News category
      ...
      const title =
        content.title.replace(/\s+/g, '-').toLowerCase() || 'bakery-content'
      ...
      setSnackbar({
        open: true,
        message:
          'Fehler beim Erstellen des Bildes. Alternative Download-Optionen sind verfügbar.',
        severity: 'error',
      })
  ...
  const isRequiredField = (field: string): boolean => {
    ...
    if (field === 'description') {
      return !['message', 'instagram-story', 'simple-square'].includes(
        templateType
      )
    }
    if (field === 'price') {
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4, position: 'relative' }}>
      ...
      <Box sx={{ mb: 4 }}>
        <TemplateSelector
          value={templateType}
          onChange={(value: string) => handleTemplateTypeChange(value as TemplateType)}
        />
      </Box>
          ...
          >
            ...
            >
              <Typography variant="h5" color="primary.main" fontWeight={500}>
                ...
                {templateType === 'instagram-square' &&
                  'Instagram Post erstellen'}
                {templateType === 'instagram-story' &&
                  'Instagram Story erstellen'}
                {templateType === 'website-banner' &&
                  'Website Banner erstellen'}
            ...
            {isComplete && (
              <Box sx={{ mt: 4, textAlign: 'center' }}>
                ...
                <Typography
                  ...
                  sx={{ mt: 1 }}
                >
                  Sie können jetzt ein weiteres Bild erstellen oder das aktuelle
                  bearbeiten.
                </Typography>
              </Box>
            )}
