...
export const importService = {
  ...
  async processReport(report: DailyReport): Promise<ImportResult> {
    ...
    try {
      ...
      for (const reportTransaction of report.transactions) {
        ...
        const userId = await mappingService.mapUser(
          reportTransaction.user,
          this.sequelize
        )
        if (!userId) {
        ...
        for (const item of reportTransaction.items) {
          ...
          const productId = await mappingService.mapProduct(
            item.product_id,
            this.sequelize
          )
          if (!productId) {
            throw new Error(
              `Product not found: ${item.product_id} (${item.product})`
            )
          }

          // Attributnamen des TransactionItem-Modells: productName und pricePerItem
          // sind Pflichtfelder (allowNull: false); ein unbekanntes `unitPrice`
          // hätte Sequelize stillschweigend verworfen.
      ...
      logger.info(
        `Successfully imported report for ${report.date}: ${transactionCount} transactions, ${itemCount} items`
      )
  ...
  async processBulkReports(reports: DailyReport[]): Promise<BulkImportResult> {
    ...
    logger.info(
      `Bulk import completed: ${result.imported} imported, ${result.skipped} skipped, ${result.failed} failed`
    )
    return result
  },
}
