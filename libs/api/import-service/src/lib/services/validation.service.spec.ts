...
describe('ValidationService', () => {
  ...
  describe('validateReportData', () => {
    ...
    it('should pass validation when all users and products exist', async () => {
      ...
      await expect(
        validationService.validateReportData(mockReport, sequelize)
  ...
  describe('validateTransactionTotals', () => {
    it('should pass when transaction totals match item totals', () => {
      ...
      expect(() =>
        validationService.validateTransactionTotals(report)
      ).not.toThrow()
    })
    ...
    it('should throw error when transaction total does not match item totals', () => {
      ...
      expect(() => validationService.validateTransactionTotals(report)).toThrow(
        'Transaction TX001 total mismatch: calculated 6.8, reported 10'
      )
    })
  })
  ...
  describe('validateDailySummary', () => {
    ...
    it('should throw error when daily summary total does not match', () => {
      const report: DailyReport = {
        ...
        daily_summary: {
          total_revenue: 15.0, // Wrong total
          cash_revenue: 10.0,
          transaction_count: 1,
          vat_totals: {},
        },
      ...
      expect(() => validationService.validateDailySummary(report)).toThrow(
        'Daily summary total mismatch: calculated 10, reported 15'
      )
    })
  })
})
