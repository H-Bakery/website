...
interface ProductionSocketCallbacks {
  ...
  onStepUpdate?: (data: { batchId: number; stepId: number } & Partial<ProductionStep>) => void
  onScheduleUpdate?: (data: { date: string } & Partial<ProductionSchedule>) => void
  onStatusUpdate?: (data: Partial<ProductionStatus>) => void
  onIssueReported?: (data: { batchId: number; issue: any }) => void
  onQualityCheck?: (data: { batchId: number; stepId: number; qualityData: any }) => void
}
...
export function useProductionSocket(
  ...
  const handleBatchUpdate = useCallback((data: { batchId: number } & Partial<ProductionBatch>) => {
    // Update batch in cache
    queryClient.setQueryData(['production', 'batch', data.batchId], (old: any) => {
      if (!old) return old
      return { ...old, ...data }
    })
  ...
  const handleStepUpdate = useCallback((data: { batchId: number; stepId: number } & Partial<ProductionStep>) => {
    // Update step in batch steps cache
    queryClient.setQueryData(['production', 'batch', data.batchId, 'steps'], (old: any) => {
  ...
  const handleScheduleUpdate = useCallback((data: { date: string } & Partial<ProductionSchedule>) => {
      ...
      return {
        ...
        data: {
          ...
          schedules: old.data.schedules.map((schedule: ProductionSchedule) =>
            schedule.scheduleDate === data.date ? { ...schedule, ...data } : schedule
          ),
        },
      }
    })
  ...
  const handleStatusUpdate = useCallback((data: Partial<ProductionStatus>) => {
    // Update status cache
  ...
  const handleIssueReported = useCallback((data: { batchId: number; issue: any }) => {
  ...
  const handleQualityCheck = useCallback((data: { batchId: number; stepId: number; qualityData: any }) => {
    // Invalidate step queries to refetch with quality data
    queryClient.invalidateQueries(['production', 'batch', data.batchId, 'steps'])
    queryClient.invalidateQueries(['production', 'batch', data.batchId])
  ...
  useEffect(() => {
    ...
    productionSocketService.on('production:schedule:update', handleScheduleUpdate)
    ...
    return () => {
      ...
      productionSocketService.off('production:schedule:update', handleScheduleUpdate)
      productionSocketService.off('production:status:update', handleStatusUpdate)
      productionSocketService.off('production:issue:reported', handleIssueReported)
      productionSocketService.off('production:quality:check', handleQualityCheck)
    }
  }, [
    handleBatchUpdate,
    handleStepUpdate,
  ...
  useEffect(() => {
    ...
    if (scheduleDate && scheduleDate !== subscriptionsRef.current.scheduleDate) {
      if (subscriptionsRef.current.scheduleDate) {
        productionSocketService.unsubscribeFromSchedule(subscriptionsRef.current.scheduleDate)
      }
    ...
    if (batchId && batchId !== subscriptionsRef.current.batchId) {
      if (subscriptionsRef.current.batchId) {
        productionSocketService.unsubscribeFromBatch(subscriptionsRef.current.batchId)
      }
  ...
  return {
    ...
    subscribeToSchedule: productionSocketService.subscribeToSchedule.bind(productionSocketService),
    unsubscribeFromSchedule: productionSocketService.unsubscribeFromSchedule.bind(productionSocketService),
    subscribeToBatch: productionSocketService.subscribeToBatch.bind(productionSocketService),
    unsubscribeFromBatch: productionSocketService.unsubscribeFromBatch.bind(productionSocketService),
    subscribeToStatus: productionSocketService.subscribeToStatus.bind(productionSocketService),
    unsubscribeFromStatus: productionSocketService.unsubscribeFromStatus.bind(productionSocketService),
  }
}