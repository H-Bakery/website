...
const AdditionalProductionManager: React.FC<AdditionalProductionManagerProps> = ({
  items,
  onItemsChange,
  onAddFromStock,
}) => {
  ...
  const getHighPriorityCount = () => {
    return items.filter(item => item.urgency === 'high' || item.urgency === 'critical').length
  }
  ...
  return (
    <Card>
      <CardHeader
        ...
        subheader={
          <Typography variant="body2" color="text.secondary">
            Artikel die während der Nachtschicht zusätzlich produziert werden sollen
          </Typography>
        }
        action={
          <Button
      ...
      <CardContent>
        {items.length === 0 ? (
          <Alert severity="info" sx={{ mb: 2 }}>
            <Typography variant="body2">
              Noch keine zusätzlichen Produktionsaufträge. Klicken Sie auf "Hinzufügen" wenn Artikel nachproduziert werden müssen.
            </Typography>
          </Alert>
        ) : (
          <>
            ...
            <Grid container spacing={2} sx={{ mb: 3 }}>
              ...
              <Grid item xs={12} sm={6} md={3}>
                <Box sx={{ textAlign: 'center', p: 2, bgcolor: 'background.default', borderRadius: 1 }}>
                  <Typography variant="h4" color="error">{highPriorityCount}</Typography>
                  <Typography variant="caption">Hohe Priorität</Typography>
                </Box>
              </Grid>
              {Object.entries(categoryCounts).slice(0, 2).map(([category, count]) => (
                <Grid item xs={12} sm={6} md={3} key={category}>
                  <Box sx={{ textAlign: 'center', p: 2, bgcolor: 'background.default', borderRadius: 1 }}>
                    <Typography variant="h4" color="secondary">{count}</Typography>
                    <Typography variant="caption">{categoryLabels[category as keyof typeof categoryLabels]}</Typography>
                  </Box>
                </Grid>
              ))}
            </Grid>
              ...
              {items
                ...
                .map((item, index) => (
                  <ListItem
                    ...
                    sx={{
                      border: 1,
                      borderColor: item.urgency === 'critical' ? 'error.main' : item.urgency === 'high' ? 'warning.main' : 'divider',
                      borderRadius: 1,
                      mb: 1,
                      bgcolor: item.urgency === 'critical' ? 'error.light' : item.urgency === 'high' ? 'warning.light' : 'background.paper',
                      '&:hover': {
                        bgcolor: item.urgency === 'critical' ? 'error.light' : item.urgency === 'high' ? 'warning.light' : 'action.hover',
                      }
                    }}
                  >
                    <Box sx={{ mr: 2 }}>
                      ...
                      secondary={
                        <Box sx={{ mt: 0.5 }}>
                          <Typography variant="body2" color="text.secondary">
                            ...
                            <strong> Kategorie:</strong> {categoryLabels[item.category]}
                            {item.requested_by && (
                              <span> | <strong>Angefordert von:</strong> {item.requested_by} um {item.requested_at}</span>
                            )}
                          </Typography>
                          {item.notes && (
        ...
        {items.length < 3 && (
          <Box sx={{ mt: 3 }}>
            <Typography variant="subtitle2" sx={{ mb: 1 }}>Häufig nachzuproduzierende Artikel:</Typography>
