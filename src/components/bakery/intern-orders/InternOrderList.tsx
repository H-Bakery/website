...
const getStatusChip = (status: InternOrder['status']) => {
  switch (status) {
    case 'pending':
      return <Chip icon={<HourglassEmptyIcon />} label="Pending" color="warning" size="small" variant="outlined" />;
    case 'in-progress':
      return <Chip icon={<PlayCircleOutlineIcon />} label="In Progress" color="info" size="small" variant="outlined" />;
    case 'done':
      return <Chip icon={<CheckCircleOutlineIcon />} label="Done" color="success" size="small" variant="outlined" />;
    case 'cancelled':
      return <Chip icon={<CancelIcon />} label="Cancelled" color="error" size="small" variant="outlined" />;
    default:
          ...
          {orders.map((order) => (
            ...
            >
              <TableCell component="th" scope="row">
                <Typography variant="subtitle2" component="div">{order.orderName}</Typography>
                <Typography variant="caption" color="text.secondary" sx={{ display: 'block', maxWidth: 250, whiteSpace: 'normal' }}>
                  {order.description}
                </Typography>
              </TableCell>
              ...
              <TableCell sx={{ textAlign: 'center' }}>
                {order.billImageUrl ? (
                  <Tooltip title="View Bill">
                    <IconButton size="small" onClick={() => window.open(order.billImageUrl, '_blank')}>
                      <ImageOutlinedIcon />
                    </IconButton>
                  </Tooltip>
                ) : (
                  <Typography variant="caption" color="text.disabled">No Bill</Typography>
                )}
              </TableCell>
              <TableCell sx={{ textAlign: 'center' }}>
                <Box sx={{ display: 'flex', justifyContent: 'center', gap: 0.5 }}>
                  {onViewOrder && (
                    <Tooltip title="View Details">
                      <IconButton size="small" onClick={() => onViewOrder(order)} color="primary">
                        <VisibilityIcon />
                      </IconButton>
                    </Tooltip>
                  )}
                  <Tooltip title="Edit Order">
                    <IconButton size="small" onClick={() => onEditOrder(order)} color="secondary">
                      <EditIcon />
                    </IconButton>
                  </Tooltip>
                  {order.status !== 'done' && order.status !== 'cancelled' && (
                    <Tooltip title="Mark as Done">
                      <IconButton size="small" onClick={() => onMarkAsDone(order.id)} color="success">
