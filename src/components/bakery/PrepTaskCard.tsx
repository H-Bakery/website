            ...
            {section.items.map((item, itemIndex) => (
              <ListItem
                ...
                sx={{ 
                  ...
                  borderRadius: 1,
                  mb: 0.5,
                  border: item.completed ? '1px solid' : '1px solid transparent',
              ...
              >
                    ...
                    <Box>
                      ...
                      {item.current_stock !== undefined && (
                        <Typography variant="caption" color="text.secondary">
                          Aktueller Bestand: {item.current_stock} {item.unit || 'Stück'}
                ...
                <ListItemSecondaryAction>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    {(item.stock_status === 'low' || item.stock_status === 'critical' || item.stock_status === 'empty') && onAddToProduction && (
                      <IconButton
                        size="small"
                        onClick={() => onAddToProduction(item.name, item.stock_status === 'empty' ? 'empty_stock' : 'low_stock')}
                        color="primary"
                        sx={{ mr: 1 }}
                      >
                        <Add />
