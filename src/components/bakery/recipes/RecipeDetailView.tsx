...
import { Recipe as RecipeType, Review as ReviewType } from '@/services/types'; // Adjust import path if necessary
...
const RecipeDetailView: React.FC<RecipeDetailViewProps> = ({ 
  ...
  const handleSaveReview = (reviewFormData: Omit<ReviewType, 'id' | 'recipeId' | 'date'>) => {
    if (editingReview) {
      ...
      onUpdateReview({ ...editingReview, ...reviewFormData, rating: Number(reviewFormData.rating) });
    } else {
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Button variant="outlined" onClick={onBack} sx={{ mb: 2 }}>Back to List</Button>
      {/* 
      ...
      */}
      ...
      <Typography variant="h5" component="h2" gutterBottom>{recipe.name}</Typography>
      <Typography variant="body1" color="text.secondary" gutterBottom>{recipe.category}</Typography>
      <Typography variant="body2" paragraph>{recipe.description || 'No description available.'}</Typography>

      <Box sx={{ mt: 2 }}>
        <Typography variant="subtitle1"><strong>Prep Time:</strong> {recipe.prepTime || 'N/A'}</Typography>
        <Typography variant="subtitle1"><strong>Cook Time:</strong> {recipe.cookTime || 'N/A'}</Typography>
        <Typography variant="subtitle1"><strong>Servings:</strong> {recipe.servings || 'N/A'}</Typography>
      </Box>
      ...
      <Box sx={{ mt: 3 }}>
        ...
        <ul>
          {recipe.ingredients.map((ing, index) => (
            <li key={index}><Typography variant="body2">{ing.quantity} {ing.name}</Typography></li>
          ))}
        </ul>
      </Box>
      ...
      {recipe.image && (
        <Box sx={{ mt: 3 }}>
          ...
          <Typography variant="body2">Image placeholder: {recipe.image}</Typography>
      ...
      <Box sx={{ mt: 4 }}>
        ...
        {(recipe.reviews || []).length === 0 && <Typography variant="body2">No reviews yet.</Typography>}
        ...
        {showReviewForm ? (
          ...
          <Button variant="contained" onClick={() => { setEditingReview(undefined); setShowReviewForm(true); }} sx={{ mt: 2 }}>Add Review</Button>
        )}
      </Box>
    </Box>
  );
