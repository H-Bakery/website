...
export const ProductionStatusPanel: React.FC<ProductionStatusPanelProps> = ({
  ...
  const getBatchStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'waiting': return <Pause color="warning" />
  ...
  const getTimelineIcon = (type: string) => {
    switch (type) {
      case 'batch_started': return <PlayArrow color="primary" />
      case 'batch_completed': return <CheckCircle color="success" />
      case 'step_completed': return <CheckCircle color="info" />
      case 'issue_reported': return <Warning color="warning" />
      case 'quality_check': return <CheckCircle color="success" />
      ...
      <Card>
        <CardContent>
          <Alert severity="error">
            Fehler beim Laden des Produktionsstatus: {(statusError as any)?.message}
          </Alert>
        </CardContent>
      </Card>
    )
  ...
  const { overview, activeBatches, pendingBatches, waitingBatches, alerts, timeline } = statusData
        ...
        <CardContent>
          ...
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
            <Box>
              ...
              <Box display="flex" alignItems="center" gap={1}>
                <Typography variant="caption" color="text.secondary">
                  Zuletzt aktualisiert: {format(new Date(statusData.lastUpdated), 'HH:mm:ss', { locale: de })}
                </Typography>
                {useWebSocket && (
                  <Chip
            ...
            <Box display="flex" gap={1} alignItems="center">
              ...
              <Tooltip title={useWebSocket && isConnected ? 'WebSocket aktiv - Automatische Updates' : 'Aktualisieren'}>
          ...
          {showAlerts && alerts.length > 0 && (
            <Box mb={3}>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                ...
                <IconButton size="small" onClick={() => setExpandedAlerts(!expandedAlerts)}>
                  {expandedAlerts ? <ExpandLess /> : <ExpandMore />}
                </IconButton>
              </Box>
              
              <Box display="flex" flexWrap="wrap" gap={1} mb={1}>
                {alerts.slice(0, expandedAlerts ? alerts.length : 3).map((alert, index) => (
                  <Alert
                    key={index}
                  ...
                  >
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        {formatDistanceToNow(parseISO(alert.timestamp), { addSuffix: true, locale: de })}
                      </Typography>
                    </Box>
                  </Alert>
                ))}
              ...
              <Grid item xs={12} md={6}>
                <Box>
                  <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                    ...
                    <IconButton size="small" onClick={() => setExpandedBatches(!expandedBatches)}>
                      {expandedBatches ? <ExpandLess /> : <ExpandMore />}
                    </IconButton>
                  </Box>
                  
                  {activeBatches.slice(0, expandedBatches ? activeBatches.length : 3).map((batch) => (
                    <Card
                      key={batch.id}
                      variant="outlined"
                      sx={{ 
                      ...
                      <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                          ...
                          <Box textAlign="right">
                            ...
                            <AvatarGroup max={3} sx={{ justifyContent: 'flex-end' }}>
                              {batch.assignedStaffIds.map((staffId) => (
                                <Avatar key={staffId} sx={{ width: 24, height: 24, fontSize: '0.7rem' }}>
                                  {staffId}
                                </Avatar>
                              ))}
                            </AvatarGroup>
                        ...
                        <Box mt={1}>
                          <LinearProgress
                            ...
                            color={batch.progress && batch.progress > 80 ? 'success' : 'primary'}
                          />
                        </Box>
                      </CardContent>
                    </Card>
              ...
              <Grid item xs={12} md={6}>
                <Box>
                  <Typography variant="subtitle2" gutterBottom>
                    Warteschlange ({pendingBatches.length + waitingBatches.length})
                  </Typography>
                  
                  {[...pendingBatches, ...waitingBatches].slice(0, 5).map((batch) => (
                    <Card
                      key={batch.id}
                      variant="outlined"
                      sx={{ 
                    ...
                    >
                      <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                        <Box display="flex" justifyContent="space-between" alignItems="center">
                          <Box>
                            ...
                            <Box display="flex" gap={0.5} mt={0.5}>
                              ...
                              <Chip
                                ...
                                size="small"
                                color={batch.priority === 'urgent' ? 'error' : 'default'}
                                variant="outlined"
                              />
                            </Box>
                          </Box>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            {format(parseISO(batch.plannedStartTime), 'HH:mm', { locale: de })}
                          </Typography>
                        </Box>
                      </CardContent>
                    </Card>
                  ...
                  {pendingBatches.length === 0 && waitingBatches.length === 0 && (
                    <Typography variant="body2" color="text.secondary" textAlign="center" py={2}>
                      ...
                      <ListItemText
                        ...
                        secondary={
                          <Typography variant="caption">
                            {formatDistanceToNow(parseISO(event.timestamp), { addSuffix: true, locale: de })}
                          </Typography>
                        }
                      />
                    </ListItem>
          ...
          {viewMode === 'list' && (
            <Box>
              <Typography variant="subtitle2" gutterBottom>
                Alle Chargen ({activeBatches.length + pendingBatches.length + waitingBatches.length})
              </Typography>
              
              <List>
                {[...activeBatches, ...pendingBatches, ...waitingBatches].map((batch, index) => (
                  <React.Fragment key={batch.id}>
                    ...
                    >
                      <ListItemAvatar>
                        <Avatar sx={{ bgcolor: getBatchStatusColor(batch.status) + '.main' }}>
                      ...
                      <ListItemText
                        ...
                        secondary={
                          <Box display="flex" justifyContent="space-between" mt={1}>
                            <Typography variant="caption">
                              {batch.plannedQuantity} {batch.unit} • Priorität: {batch.priority}
                            </Typography>
                            <Typography variant="caption">
                              {format(parseISO(batch.plannedStartTime), 'HH:mm', { locale: de })} - 
                              {format(parseISO(batch.plannedEndTime), 'HH:mm', { locale: de })}
                            </Typography>
                          </Box>
                        }
                      />
                    </ListItem>
                    {index < activeBatches.length + pendingBatches.length + waitingBatches.length - 1 && (
