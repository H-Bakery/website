...
import { format, parseISO, addHours, differenceInMinutes, startOfDay, endOfDay } from 'date-fns'
...
export const ProductionScheduleBoard: React.FC<ProductionScheduleBoardProps> = ({
  selectedDate,
  viewMode,
  onDateChange,
  onViewModeChange,
  ...
  const handleMenuClick = (event: React.MouseEvent<HTMLElement>, batch: ProductionBatch) => {
  ...
  const handleCreateBatch = async () => {
    try {
      await createBatchMutation.mutateAsync({
        ...newBatchData,
        plannedStartTime: `${format(selectedDate, 'yyyy-MM-dd')}T${newBatchData.plannedStartTime}`,
        plannedEndTime: `${format(selectedDate, 'yyyy-MM-dd')}T${newBatchData.plannedStartTime}`, // Will be calculated by backend
      })
  ...
  if (schedulesError || batchesError) {
    return (
      <Alert severity="error" sx={{ m: 2 }}>
        Fehler beim Laden des Produktionsplans: {(schedulesError as any)?.message || (batchesError as any)?.message}
      </Alert>
    )
  }
    ...
    return (
      <Box p={3} textAlign="center">
        <Typography variant="h6" color="text.secondary">
          Kein Produktionsplan für {format(selectedDate, 'dd.MM.yyyy', { locale: de })}
        </Typography>
        <Button
          variant="contained"
          startIcon={<Add />}
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} md={8}>
              <Typography variant="h6">
                Produktionsplan - {format(selectedDate, 'dd.MM.yyyy', { locale: de })}
              </Typography>
            ...
            <Grid item xs={12} md={4}>
              <Box display="flex" gap={1} justifyContent="flex-end">
                <Chip
                  icon={<CheckCircle />}
                  label={`${currentSchedule.completionPercentage?.toFixed(0) || 0}% abgeschlossen`}
                  color={currentSchedule.completionPercentage && currentSchedule.completionPercentage > 80 ? 'success' : 'default'}
                  size="small"
                />
                <Chip
                  icon={<Schedule />}
                  label={`${currentSchedule.capacityUtilization?.toFixed(0) || 0}% Auslastung`}
                  color={currentSchedule.capacityUtilization && currentSchedule.capacityUtilization > 90 ? 'warning' : 'default'}
                  size="small"
                />
              </Box>
            </Grid>
                ...
                >
                  <Card
                    sx={{
                      height: '100%',
                      cursor: 'pointer',
                      bgcolor: batch.status === 'in_progress' ? 'primary.light' :
                              batch.status === 'completed' ? 'success.light' :
                              batch.status === 'failed' ? 'error.light' :
                              'grey.100',
                      '&:hover': {
                        bgcolor: batch.status === 'in_progress' ? 'primary.main' :
                                batch.status === 'completed' ? 'success.main' :
                                batch.status === 'failed' ? 'error.main' :
                                'grey.200',
                      },
                    }}
                  >
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={(e) => setNewBatchData({ ...newBatchData, workflowId: e.target.value })}
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField
            ...
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  label="Priorität"
                  onChange={(e) => setNewBatchData({ ...newBatchData, priority: e.target.value as any })}
                >
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={(e) => setNewBatchData({ ...newBatchData, plannedStartTime: e.target.value })}
