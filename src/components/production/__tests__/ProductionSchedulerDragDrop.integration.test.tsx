...
import { simulateDragAndDrop, createMockBatch, createMockSchedule } from './dragDropTestUtils'
...
describe('ProductionSchedulerDragDrop - Integration Tests', () => {
  ...
  it('renders the scheduler with batches', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Drag & Drop Produktionsplanung')).toBeInTheDocument()
    })
  ...
  it('updates UI in real-time with WebSocket events', async () => {
    ...
    ;(useProductionSocket as jest.Mock).mockImplementation((options, callbacks) => {
  ...
  it('handles error scenarios gracefully', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Fehler beim Laden: Failed to load schedules/)).toBeInTheDocument()
    })
  })
  ...
  it('shows no schedule message when no schedule exists', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Kein Produktionsplan für diesen Tag')).toBeInTheDocument()
    })
