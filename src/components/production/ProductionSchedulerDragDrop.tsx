...
import { format, parseISO, addHours, differenceInMinutes, startOfDay, endOfDay, addMinutes, isSameDay } from 'date-fns'
...
export const ProductionSchedulerDragDrop: React.FC<ProductionSchedulerDragDropProps> = ({
  selectedDate,
  viewMode,
  onDateChange,
  onViewModeChange,
  ...
  const currentSchedule = useMemo(() => {
    return schedules.find(schedule => 
      format(parseISO(schedule.scheduleDate), 'yyyy-MM-dd') === format(selectedDate, 'yyyy-MM-dd')
    )
  ...
  const timelineHours = useMemo(() => {
    ...
    const startTime = parseISO(`${currentSchedule.scheduleDate}T${currentSchedule.workdayStartTime}`)
    const endTime = parseISO(`${currentSchedule.scheduleDate}T${currentSchedule.workdayEndTime}`)
  ...
  // Handle drag start
  const handleDragStart = useCallback((batch: ProductionBatch, index: number) => {
    setDragState({
      isDragging: true,
      draggedBatch: batch,
      draggedIndex: index,
  ...
  const handleDrop = useCallback(async (batchId: number, newStartTime: Date) => {
    ...
    if (conflicts.length > 0) {
      ...
      setSnackbarMessage('Zeitkonflikt! Bitte wählen Sie einen anderen Zeitslot.')
      handleDragEnd()
      return
    }
    ...
    handleDragEnd()
  }, [batches, checkConflicts, updateBatchMutation, undoStack, refetchBatches, handleDragEnd])
  ...
  const getBatchPosition = useCallback((batch: ProductionBatch) => {
    if (!currentSchedule) return { left: 0, width: 0 }
    
    const scheduleStart = parseISO(`${currentSchedule.scheduleDate}T${currentSchedule.workdayStartTime}`)
    const scheduleEnd = parseISO(`${currentSchedule.scheduleDate}T${currentSchedule.workdayEndTime}`)
    const batchStart = parseISO(batch.plannedStartTime)
    const batchEnd = parseISO(batch.plannedEndTime)
    ...
    return {
      ...
      width: Math.min(100 - Math.max(0, (startOffset / totalMinutes) * 100), (duration / totalMinutes) * 100),
    }
  }, [currentSchedule])
  ...
  if (schedulesError || batchesError) {
    return (
      <Alert severity="error" sx={{ m: 2 }}>
        Fehler beim Laden: {(schedulesError as any)?.message || (batchesError as any)?.message}
      </Alert>
    )
  }
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} md={6}>
              ...
              <Typography variant="body2" color="text.secondary">
                ...
                {currentSchedule ? ` ${currentSchedule.workdayStartTime} - ${currentSchedule.workdayEndTime} Uhr` : ' Kein Plan'}
              </Typography>
            </Grid>
                ...
                <Tooltip title="Ansicht wechseln">
                  <IconButton
                    ...
                    onClick={() => onViewModeChange({
                      ...viewMode,
                      type: viewMode.type === 'timeline' ? 'kanban' : 'timeline'
                    })}
                  >
                    {viewMode.type === 'timeline' ? <ViewModule /> : <ViewStream />}
                  </IconButton>
                </Tooltip>
          ...
          <Card>
            <CardContent>
              {!currentSchedule ? (
                ...
                <>
                  {/* Timeline Header */}
                  <Box position="relative" height={40} mb={2} bgcolor="grey.50" borderRadius={1}>
                  ...
                  {/* Resource Rows */}
                  {Object.entries(batchesByResource).map(([resource, resourceBatches]) => (
                    <Box key={resource} mb={2}>
                      <Typography variant="subtitle2" gutterBottom>
                        {resource === 'unassigned' ? 'Nicht zugewiesen' : resource}
                      </Typography>
                      
                      <Box position="relative" minHeight={80} bgcolor="grey.50" borderRadius={1} p={1}>
                        {/* Drop Zones */}
                        <Box 
                          position="absolute" 
                          top={0} 
                          ...
                          {timelineHours.map((hour, index) => (
                            <Box key={index} flex={1} position="relative">
                              <ProductionTimelineDropZone
                                ...
                                scheduleStartTime={parseISO(`${currentSchedule.scheduleDate}T${currentSchedule.workdayStartTime}`)}
                                scheduleEndTime={parseISO(`${currentSchedule.scheduleDate}T${currentSchedule.workdayEndTime}`)}
                        ...
                        {resourceBatches.map((batch, index) => {
                          ...
                          const isConflict = conflictBatches.includes(batch.id)
                          ...
                          return (
                            <Box
                              ...
                              top={10}
                              height={60}
                              zIndex={dragState.draggedBatch?.id === batch.id ? 1000 : 10}
                              sx={{
                                transition: dragState.isDragging ? 'none' : 'all 0.3s ease',
                                opacity: isConflict ? 0.6 : 1,
                                filter: isConflict ? 'brightness(1.2)' : 'none',
                              }}
                            >
                              <DraggableProductionBatch
                                ...
                                isDragging={dragState.draggedBatch?.id === batch.id}
                              />
                            </Box>
                          )
                        })}
                ...
                // Kanban View
                <Grid container spacing={2}>
                  {['planned', 'ready', 'in_progress', 'completed'].map(status => (
