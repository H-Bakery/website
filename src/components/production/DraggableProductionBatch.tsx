...
interface DraggableProductionBatchProps {
  ...
  index: number
  onMenuClick: (event: React.MouseEvent<HTMLElement>, batch: ProductionBatch) => void
...
export const DraggableProductionBatch: React.FC<DraggableProductionBatchProps> = ({
  batch,
  index,
  onMenuClick,
  onDragStart,
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'waiting': return <Schedule fontSize="small" color="warning" />
      default: return null
    }
  }
  ...
  const handleDragStart = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    e.dataTransfer.setData('duration', batch.estimatedDurationMinutes.toString())
                ...
                >
                  <Box
                    sx={{
                      ...
                      height: '100%',
                      bgcolor: batch.status === 'failed' ? 'error.main' : 'primary.main',
