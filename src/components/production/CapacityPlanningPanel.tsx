...
import { useCapacityAnalysis, useOptimizeSchedule } from '../../hooks/useProduction'
import {
  CapacityAnalysis,
  StaffCapacity,
  EquipmentCapacity,
...
export const CapacityPlanningPanel: React.FC<CapacityPlanningPanelProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'overview' | 'staff' | 'equipment'>('overview')
  const [selectedWorker, setSelectedWorker] = useState<WorkerCapacity | null>(null)
  const [selectedEquipment, setSelectedEquipment] = useState<EquipmentStation | null>(null)
  ...
  const getBottleneckIcon = (type: string, severity: string) => {
    if (type === 'staff') return <People color={severity === 'high' ? 'error' : 'warning'} />
    if (type === 'equipment') return <Build color={severity === 'high' ? 'error' : 'warning'} />
      ...
      <Card>
        <CardContent>
          <Alert severity="error">
            Fehler beim Laden der Kapazitätsanalyse: {(capacityError as any)?.message}
          </Alert>
        </CardContent>
      </Card>
    )
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
          <Box>
            <Typography variant="h6">
              Kapazitätsplanung - {format(selectedDate, 'dd.MM.yyyy', { locale: de })}
            </Typography>
        ...
        <Grid container spacing={2} mb={3}>
          <Grid item xs={12} sm={6} md={3}>
            <Box textAlign="center" p={2} bgcolor="primary.light" borderRadius={1}>
              <People sx={{ fontSize: 40, color: 'primary.contrastText', mb: 1 }} />
              ...
              <LinearProgress
                ...
                sx={{ mt: 1, bgcolor: 'primary.dark', '& .MuiLinearProgress-bar': { bgcolor: 'white' } }}
              />
              <Typography variant="caption" color="primary.contrastText">
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Box textAlign="center" p={2} bgcolor="secondary.light" borderRadius={1}>
              <Build sx={{ fontSize: 40, color: 'secondary.contrastText', mb: 1 }} />
              ...
              <LinearProgress
                ...
                sx={{ mt: 1, bgcolor: 'secondary.dark', '& .MuiLinearProgress-bar': { bgcolor: 'white' } }}
              />
              <Typography variant="caption" color="secondary.contrastText">
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Box textAlign="center" p={2} bgcolor="success.light" borderRadius={1}>
              <Schedule sx={{ fontSize: 40, color: 'success.contrastText', mb: 1 }} />
                ...
                <Table size="small">
                  ...
                  <TableBody>
                    {Object.entries(
                      ...
                      <TableRow key={role}>
                        ...
                        <TableCell align="center">
                          <LinearProgress
                            variant="determinate"
                            value={calculateUtilization(data.hours * 0.8, data.hours)}
                            sx={{ width: 60, mx: 'auto' }}
                          />
                        </TableCell>
                      </TableRow>
                    ...
                    <TableRow>
                      ...
                      <TableCell align="center">
                        <Chip label={staffCapacity.availableWorkers} size="small" color="primary" />
                      </TableCell>
                      <TableCell align="center"><strong>{totalStaffHours}h</strong></TableCell>
                      <TableCell align="center">
                        <LinearProgress
                          variant="determinate"
                          value={staffUtilization}
                  ...
                  <TableBody>
                    {Object.entries(
                      equipmentCapacity.stations.reduce((acc, station) => {
                        if (!acc[station.type]) {
                          acc[station.type] = { count: 0, capacity: 0, available: 0 }
                        }
                      ...
                      <TableRow key={type}>
                        ...
                        <TableCell align="center">
                          <Box display="flex" alignItems="center" justifyContent="center">
                            ...
                            <LinearProgress
                              variant="determinate"
                              value={calculateUtilization(data.capacity - data.available, data.capacity)}
                              sx={{ width: 60, ml: 1 }}
                            />
                          </Box>
                        </TableCell>
                    ...
                    <TableRow>
                      <TableCell><strong>Gesamt</strong></TableCell>
                      <TableCell align="center">
                        <Chip label={equipmentCapacity.totalStations} size="small" color="secondary" />
                      </TableCell>
                      <TableCell align="center"><strong>{equipmentCapacity.totalCapacity}</strong></TableCell>
                      <TableCell align="center">
                        <Box display="flex" alignItems="center" justifyContent="center">
                          <strong>{equipmentCapacity.totalAvailableHours}h</strong>
                          <LinearProgress
                            variant="determinate"
                            value={equipmentUtilization}
                            sx={{ width: 60, ml: 1 }}
                    ...
                    secondary={
                      <Box>
                        <Typography variant="caption" display="block">
                          {format(parseISO(worker.startTime), 'HH:mm')} - {format(parseISO(worker.endTime), 'HH:mm')} ({worker.hours}h)
                        </Typography>
              ...
              {equipmentCapacity.stations.map((station) => (
                <Grid item xs={12} sm={6} md={4} key={station.id}>
                  <Card
                    ...
                    sx={{
                      cursor: 'pointer',
                      bgcolor: selectedEquipment?.id === station.id ? 'action.selected' : 'background.paper',
                  ...
                  >
                    <CardContent>
                      ...
                      <Box>
                        ...
                        <LinearProgress
                          variant="determinate"
                          value={calculateUtilization(station.capacity - station.availableHours, station.capacity)}
                          sx={{ mt: 0.5, mb: 0.5 }}
                        />
