...
const TemplateSelector: React.FC<TemplateSelectorProps> = ({ value, onChange }) => {
  ...
  const getCurrentPlatform = (templateType: TemplateType): PlatformCategory => {
    if (['facebook-post', 'instagram-square', 'instagram-story', 'simple-square'].includes(templateType)) {
      return 'social'
    }
    ...
    >
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body2" sx={{ color: 'text.secondary', mb: 1 }}>
          {selectedPlatform === 'classic' && 'Traditionelle Bäckerei-Templates für allgemeine Verwendung'}
          {selectedPlatform === 'social' && 'Optimiert für Facebook und Instagram'}
          {selectedPlatform === 'website' && 'Banner und Karten für Ihre Website'}
        </Typography>
      </Box>
