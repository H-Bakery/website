  ...
  const loadWeeklyData = async () => {
    try {
      ...
      const dailySummaries: DaySummary[] = weekDates.map(date => {
        const dayEntries = unsoldData.filter(entry => entry.date === date)
        return {
          date,
          totalUnsold: dayEntries.reduce((sum, entry) => sum + entry.quantity, 0),
      ...
      const weekEntries = unsoldData.filter(entry => weekDates.includes(entry.date))
