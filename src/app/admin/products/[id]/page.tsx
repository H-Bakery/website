...
export default function ProductEditPage() {
  ...
  const [originalData, setOriginalData] = useState<Partial<Product>>(initialFormData);
  ...
  const {
    ...
    ['product', productId],
    () => bakeryAPI.getProductById(productId!),
    {
      enabled: !!productId,
      onSuccess: (data) => {
        setFormData(data);
  ...
  const updateMutation = useMutation(
      ...
      bakeryAPI.updateProduct(updatedProduct.id, updatedProduct.data),
    {
      onSuccess: (updatedProductData) => {
        ...
        queryClient.setQueryData(['product', productId], updatedProductData); // Update cache for this product
  ...
  const handleSave = () => {
    ...
    const dataToSave: Partial<Omit<Product, 'id'>> = {
      ...
      stock: formData.stock !== undefined ? parseInt(String(formData.stock), 10) : 0,
      dailyTarget: formData.dailyTarget !== undefined ? parseInt(String(formData.dailyTarget), 10) : 0,
    };
    // Remove id from dataToSave if it exists, as it's passed separately
    delete (dataToSave as Partial<Product>).id;


    updateMutation.mutate({ id: productId, data: dataToSave });
  };
  ...
  if (isErrorProduct || !product) {
    return (
      <Container sx={{ mt: 4 }}>
        ...
        <Button startIcon={<ArrowBackIcon />} onClick={() => router.push('/admin/products')} sx={{ mt: 2 }}>
          Back to Products
        </Button>
      </Container>
    );
      ...
      <Paper elevation={3} sx={{ p: { xs: 2, sm: 3 } }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
          ...
          <Typography variant="h5" component="h1" sx={{ ml: 1 }}>
            Edit Product: {originalData.name || `ID: ${productId}`}
          </Typography>
        </Box>

        {successAlert && <Alert severity="success" sx={{ mb: 2 }}>{successAlert}</Alert>}
        {errorAlert && <Alert severity="error" sx={{ mb: 2 }}>{errorAlert}</Alert>}

        <Grid container spacing={3}>
          <Grid item xs={12} md={4}>
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              {imagePreviewError || !formData.image ? (
                 <Avatar sx={{ width: 180, height: 180, mb: 2, fontSize: '4rem' }} variant="rounded">
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  size="small"
                  InputProps={{
                    startAdornment: <InputAdornment position="start">{formatter.currencySymbol || '€'}</InputAdornment>,
          ...
          <Grid item xs={12}>
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 2 }}>
              ...
              <Button
                ...
                onClick={handleSave}
                startIcon={<SaveIcon />}
                disabled={!isDirty || updateMutation.isLoading || isLoadingProduct}
              >
                Save Changes
              </Button>
            </Box>
