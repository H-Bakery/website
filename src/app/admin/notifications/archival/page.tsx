...
const NotificationArchivalPage: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      setErrorMessage(error.message || 'Fehler beim Laden der Archivierungs-Einstellungen');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 4 }}>
        Verwalten Sie die automatische Archivierung und Bereinigung von Benachrichtigungen
      </Typography>
                ...
                {status?.isRunning ? (
                  <Button
                    ...
                    onClick={() => setConfirmDialog({
                      ...
                      message: 'Möchten Sie den Archivierungs-Service wirklich stoppen?',
                  ...
                  <Grid item xs={12} sm={6}>
                    <Stack direction="row" alignItems="center" spacing={1}>
                      ...
                      <Box>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {format(new Date(nextRuns.archival), 'dd.MM.yyyy HH:mm')}
                          {' '}({formatDistanceToNow(new Date(nextRuns.archival), { addSuffix: true, locale: de })})
                        </Typography>
                      </Box>
                    </Stack>
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <Stack direction="row" alignItems="center" spacing={1}>
                      ...
                      <Box>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {format(new Date(nextRuns.cleanup), 'dd.MM.yyyy HH:mm')}
                          {' '}({formatDistanceToNow(new Date(nextRuns.cleanup), { addSuffix: true, locale: de })})
                        </Typography>
                      </Box>
                    </Stack>
                  </Grid>
              ...
              <Stack direction="row" spacing={2}>
                <Button
                  ...
                  onClick={() => setConfirmDialog({
                    ...
                    message: 'Möchten Sie die Archivierung jetzt manuell ausführen?',
                ...
                <Button
                  ...
                  onClick={() => setConfirmDialog({
                    ...
                    message: 'Möchten Sie die Bereinigung jetzt manuell ausführen? Diese Aktion kann nicht rückgängig gemacht werden.',
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6}>
                <FormControl fullWidth>
                  ...
                  >
                    {['staff', 'order', 'system', 'inventory', 'general'].map((category) => (
