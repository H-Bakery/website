...
import type { Notification, ArchiveResult, ArchiveStats } from '../../../../types/notification';
...
const NotificationArchivePage: React.FC = () => {
  ...
  const loadArchivedNotifications = useCallback(async (resetPage = false) => {
    try {
      setLoading(true);
      ...
      const result: ArchiveResult = await bakeryAPI.getArchivedNotifications(options);
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      const result: ArchiveResult = await bakeryAPI.searchNotifications(searchQuery, options);
  ...
  const handleBulkRestore = async () => {
    ...
    try {
      ...
      setSuccessMessage(`${selectedIds.size} Benachrichtigungen wiederhergestellt`);
  ...
  const handleSelectAll = () => {
    const currentNotifications = tabValue === 0 ? archivedNotifications : searchResults;
  ...
  const NotificationItem: React.FC<{ notification: Notification; showSelect?: boolean }> = ({ 
    notification, 
    showSelect = true 
  }) => (
    <ListItem
          ...
          <>
            ...
            <Typography component="span" variant="caption" color="text.secondary">
              {notification.archived && notification.archivedAt ? (
                <>
                  Archiviert {formatDistanceToNow(new Date(notification.archivedAt), {
                    addSuffix: true,
                    locale: de,
                  })} 
                  ({format(new Date(notification.archivedAt), 'dd.MM.yyyy HH:mm')})
                </>
              ) : (
  ...
  return (
    <Box>
        ...
        <Box sx={{ borderBottom: 1, borderColor: 'divider', px: 2, pt: 2 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
            ...
            <Stack direction="row" spacing={1}>
              <Button
                size="small"
                startIcon={selectedIds.size > 0 ? <DeselectIcon /> : <SelectAllIcon />}
        ...
        <TabPanel value={tabValue} index={1}>
          {searchLoading ? (
            ...
            <Box sx={{ p: 4, textAlign: 'center' }}>
              ...
              <Typography color="text.secondary">
                {searchQuery ? 'Keine Suchergebnisse gefunden' : 'Geben Sie einen Suchbegriff ein'}
              </Typography>
            </Box>
          ) : (
            <List sx={{ p: 2 }}>
              {searchResults.map((notification) => (
                <NotificationItem key={notification.id} notification={notification} showSelect={false} />
              ))}
            </List>
          )}
        </TabPanel>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Diese Aktion kann nicht rückgängig gemacht werden. Die Benachrichtigung wird dauerhaft gelöscht.
          </Typography>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            onClick={() => notificationToDelete && handlePermanentDelete(notificationToDelete)}
            color="error"
            variant="contained"
          >
            Endgültig löschen
