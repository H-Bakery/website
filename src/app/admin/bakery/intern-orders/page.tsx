...
export default function InternOrdersPage() {
  ...
  const handleSubmitForm = async (formData: Omit<InternOrder, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      ...
      setError('Failed to save intern order. Please check the details and try again.');
  ...
  if (isLoading && orders.length === 0) { // Show loader only on initial load or full processing
    return (
  ...
  return (
    <Container maxWidth="lg" sx={{ py: { xs: 2, md: 4 } }}>
      ...
      {showForm ? (
        ...
        <Paper elevation={0} sx={{ p: { xs: 1, md: 2 }, backgroundColor: 'transparent' }}>
          <Box
            sx={{
              display: 'flex',
          ...
          {isLoading && <CircularProgress size={24} sx={{position: 'absolute', top: '90px', right: '40px'}}/>}
          <InternOrderList
            orders={orders}
