...
import { Recipe as RecipeType, Review as ReviewType } from '@/services/types';
...
export default function RecipeManagementPage() {
  ...
  const handleAddRecipe = (newRecipeData: Omit<RecipeType, 'id' | 'reviews'>) => {
  ...
  const handleAddReviewToRecipe = (recipeId: string, reviewData: Omit<ReviewType, 'id' | 'recipeId' | 'date'>) => {
  ...
  const handleUpdateReviewInRecipe = (recipeId: string, updatedReview: ReviewType) => {
    setRecipes(prevRecipes =>
      prevRecipes.map(recipe => {
        if (recipe.id === recipeId) {
  ...
  if (selectedRecipe) {
    return (
      <RecipeDetailView
        ...
        onAddReview={(reviewData) => handleAddReviewToRecipe(selectedRecipe.id, reviewData)}
        onUpdateReview={(reviewData) => handleUpdateReviewInRecipe(selectedRecipe.id, reviewData)}
        onDeleteReview={(reviewId) => handleDeleteReviewFromRecipe(selectedRecipe.id, reviewId)}
  ...
  return (
    <Container>
      <Typography variant="h4" component="h1" gutterBottom>Recipe Management</Typography>
      ...
      <Box marginTop={2}>
        {recipes.map(recipe => (
          <RecipeListItem key={recipe.id} recipe={recipe} onClick={() => handleSelectRecipe(recipe)} />
        ))}
      </Box>
    </Container>
  );
