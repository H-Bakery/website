...
interface ProductionSocketEvents {
  // Batch events
  'production:batch:update': (data: { batchId: number } & Partial<ProductionBatch>) => void
  'production:step:update': (data: { batchId: number; stepId: number } & Partial<ProductionStep>) => void
  
  // Schedule events
  'production:schedule:update': (data: { date: string } & Partial<ProductionSchedule>) => void
  ...
  // Quality events
  'production:quality:check': (data: { batchId: number; stepId: number; qualityData: any }) => void
}

export type ProductionSocketEventHandler<T extends keyof ProductionSocketEvents> = 
