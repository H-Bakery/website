...
export const internOrderService = {
  ...
  addInternOrder: async (orderData: Omit<InternOrder, 'id' | 'createdAt' | 'updatedAt'>): Promise<InternOrder> => {
  ...
  updateInternOrder: async (id: string, updates: Partial<Omit<InternOrder, 'id' | 'createdAt'>>): Promise<InternOrder | null> => {
    await new Promise(resolve => setTimeout(resolve, 300));
