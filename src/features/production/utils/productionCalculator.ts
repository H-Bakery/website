    ...
    Object.entries(orders).forEach(([product, quantity]) => {
      if (this.recipes[product]) {
        ...
        if (doughPiece) {
          ...
          doughPieces[recipe.doughPiece] = (doughPieces[recipe.doughPiece] || 0) + (quantity * doughPiece.count);

          // Calculate filling needs
          if (recipe.filling && recipe.fillingAmount) {
            fillings[recipe.filling] = (fillings[recipe.filling] || 0) + (recipe.fillingAmount * quantity);
          }
        }
      }
    });
