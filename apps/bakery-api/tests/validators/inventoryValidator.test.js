...
describe('Inventory Validator', () => {
  describe('Inventory Creation Rules', () => {
    ...
    it('should reject invalid unit', async () => {
      ...
      expect(response.status).toBe(422)
      expect(response.body.errors.unit).toContain('Invalid unit')
    })
    ...
    it('should reject negative quantity', async () => {
      ...
      expect(response.status).toBe(422)
      expect(response.body.errors.quantity).toContain(
  ...
  describe('Stock Adjustment Rules', () => {
    ...
    it('should accept valid stock adjustment', async () => {
      const response = await request(app).patch('/test/1').send({
        adjustment: 50,
    ...
    it('should reject zero adjustment', async () => {
      ...
      expect(response.status).toBe(422)
      expect(response.body.errors.adjustment).toContain(
  ...
  describe('Bulk Stock Adjustment Rules', () => {
    ...
    it('should reject empty adjustments array', async () => {
      ...
      expect(response.status).toBe(422)
      expect(response.body.errors.adjustments).toContain(
    ...
    it('should reject too many adjustments', async () => {
      ...
      const response = await request(app).post('/test').send({ adjustments })

      expect(response.status).toBe(422)
      expect(response.body.errors.adjustments).toContain(
        'Cannot process more than 100 adjustments at once'
      )
    })
