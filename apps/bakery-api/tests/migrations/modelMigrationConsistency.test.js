...
// Skip: requires sequelize-cli migrations infrastructure
describe.skip('Model-Migration Consistency Tests', () => {
  beforeEach(async () => {
  ...
  describe('Schema Consistency', () => {
    ...
    it('should create the same table structure via migrations and sync', async () => {
      ...
      const tablesFromMigrations = await sequelize
        .getQueryInterface()
        .showAllTables()
      ...
      expect(tablesFromMigrations.length).toBeGreaterThanOrEqual(
        tablesFromSync.length
      )
  ...
  describe('Model Validation with Migration Schema', () => {
    ...
    it('should handle soft deletes properly', async () => {
      ...
      if (userAttributes.deletedAt) {
        ...
        const userInDb = await models.User.findByPk(user.id, {
          paranoid: false,
        })
