...
describe('Recipe Parser Utility', () => {
  ...
  describe('generateSlug', () => {
    it('should generate URL-friendly slugs', () => {
      ...
      expect(recipeParser.generateSlug('Recipe with Numbers 123')).toBe(
        'recipe-with-numbers-123'
      )
      expect(recipeParser.generateSlug('  Trimmed  Recipe  ')).toBe(
        'trimmed-recipe'
      )
    })
    ...
    it('should handle German umlauts', () => {
      ...
      expect(recipeParser.generateSlug('Brötchen mit Öl')).toBe(
        'broetchen-mit-oel'
      )
    ...
    it('should handle special characters', () => {
      ...
      expect(recipeParser.generateSlug('50% Whole Wheat')).toBe(
        '50-whole-wheat'
      )
  ...
  describe('parseRecipeFile', () => {
    beforeEach(() => {
      const recipesPath = path.join(__dirname, '../../../hq/data/recipes')
      mockFs({
        [recipesPath]: {
          cakes: {
            'chocolate-cake.md': `---
...
`,
    ...
    it('should handle files without frontmatter', async () => {
      ...
      const recipe = await recipeParser.parseRecipeFile(
        path.join(recipesPath, 'simple.md')
      )
...
Bread content`,
  ...
  describe('createRecipe', () => {
    ...
    it('should throw error if recipe already exists', async () => {
      ...
      await expect(recipeParser.createRecipe(recipeData)).rejects.toThrow(
        'Recipe with this slug already exists'
      )
    })
  })
...
Original content`,
    ...
    it('should update an existing recipe', async () => {
      ...
      const result = await recipeParser.updateRecipe(
        'chocolate-cake',
        updatedData
      )
    ...
    it('should move recipe to new category if changed', async () => {
      ...
      const recipesPath = path.join(__dirname, '../../../hq/data/recipes')
      await expect(
        fs.access(path.join(recipesPath, 'cakes', 'chocolate-cake.md'))
      ).rejects.toThrow()
