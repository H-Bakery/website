...
class InventoryService {
    ...
    try {
      ...
      if (newQuantity < 0) {
        const error = new Error(
          `Insufficient stock. Available: ${oldQuantity}, Requested change: ${change}`
        )
  ...
  async bulkAdjustStock(adjustments, reason = 'Bulk adjustment') {
    ...
    try {
      logger.info(
        `Starting bulk stock adjustment for ${adjustments.length} items`,
        { reason }
      )

      for (const adjustment of adjustments) {
        try {
          const item = await this.adjustStockLevel(
            adjustment.id,
            adjustment.change,
            reason
          )
