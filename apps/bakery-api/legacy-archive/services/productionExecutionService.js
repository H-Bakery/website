...
class ProductionExecutionService {
  ...
  async advanceWorkflow(batchId, currentStepIndex) {
    try {
      ...
      const nextStep = batch.ProductionSteps.find(
  ...
  async pauseBatch(batchId, reason, userId) {
    try {
      ...
      // Pause active steps
      const activeStep = batch.ProductionSteps.find(
  ...
  async resumeBatch(batchId, userId) {
    try {
      ...
      // Resume active step
      const waitingStep = batch.ProductionSteps.find(
  ...
  async enrichSingleBatch(batch) {
    ...
    if (batch.ProductionSteps) {
      ...
      const completedSteps = batch.ProductionSteps.filter(
      ...
      // Current step info
      const currentStep = batch.ProductionSteps.find(
    ...
    if (batch.plannedEndTime) {
      ...
      enriched.delayMinutes = enriched.isDelayed
        ? Math.round((now - plannedEnd) / (1000 * 60))
        : 0
    }
    ...
    if (batch.actualStartTime) {
      ...
      enriched.actualDurationMinutes = Math.round(
        (new Date(actualEnd) - new Date(batch.actualStartTime)) / (1000 * 60)
      )
    }
  ...
  async enrichStepData(step) {
    ...
    if (step.actualStartTime) {
      ...
      enriched.actualDurationMinutes = Math.round(
        (new Date(actualEnd) - new Date(step.actualStartTime)) / (1000 * 60)
      )
    }
    ...
    for (const batch of batches) {
      ...
      if (
        ...
        const delayMinutes = Math.round(
          (now - new Date(batch.plannedEndTime)) / (1000 * 60)
        )
        alerts.push({
          type: 'delay',
  ...
  async validateStepPreconditions(step, batch) {
    ...
    const previousSteps = batch.ProductionSteps.filter(
    ...
    if (incompletePrevious.length > 0) {
      return {
        valid: false,
        reason: `Previous steps must be completed: ${incompletePrevious
          .map((s) => s.stepName)
          .join(', ')}`,
      }
    }
