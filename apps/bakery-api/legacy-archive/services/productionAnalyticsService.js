...
class ProductionAnalyticsService {
    ...
    completedBatches.forEach((batch) => {
      if (batch.plannedStartTime && batch.plannedEndTime) {
        const plannedDuration =
          new Date(batch.plannedEndTime) - new Date(batch.plannedStartTime)
        const actualDuration =
          new Date(batch.actualEndTime) - new Date(batch.actualStartTime)
  ...
  async calculateQualityMetrics(batches) {
    const totalSteps = batches.reduce(
      (sum, batch) => sum + (batch.ProductionSteps?.length || 0),
      0
    )
    ...
    const stepsWithIssues = batches.reduce(
      (sum, batch) =>
        sum +
        (batch.ProductionSteps?.filter((step) => step.hasIssues).length || 0),
      0
    )
    ...
    const qualityChecksCompleted = batches.reduce(
      (sum, batch) =>
        sum +
        (batch.ProductionSteps?.filter((step) => step.qualityCheckCompleted)
          .length || 0),
      0
    )
  ...
  async calculateTimingMetrics(batches) {
    ...
    const currentlyDelayed = batches.filter(
      (batch) =>
    ...
    return {
      onTimePercentage:
        completedBatches.length > 0
          ? Math.round((onTimeBatches / completedBatches.length) * 100)
          : 0,
      delayedPercentage:
        completedBatches.length > 0
          ? Math.round((delayedBatches / completedBatches.length) * 100)
          : 0,
      earlyPercentage:
        completedBatches.length > 0
          ? Math.round((earlyBatches / completedBatches.length) * 100)
          : 0,
      averageDelayMinutes:
        delayedBatches > 0 ? Math.round(totalDelayMinutes / delayedBatches) : 0,
      currentlyDelayed,
      onTimeBatches,
      delayedBatches,
      earlyBatches,
  ...
  async calculateThroughputMetrics(batches, groupBy = 'day') {
    ...
    const avgBatchesPerPeriod =
      totalPeriods > 0
        ? throughputArray.reduce((sum, d) => sum + d.batches, 0) / totalPeriods
        : 0
    const avgQuantityPerPeriod =
      totalPeriods > 0
        ? throughputArray.reduce((sum, d) => sum + d.actualQuantity, 0) /
          totalPeriods
        : 0
    ...
    return {
      ...
      summary: {
        ...
        peakQuantity: Math.max(
          ...throughputArray.map((d) => d.actualQuantity),
          0
        ),
      },
    }
  ...
  async calculateWorkflowMetrics(batches) {
    ...
    const workflowMetrics = Array.from(workflowData.values()).map((data) => ({
      ...
      failureRate:
        data.totalBatches > 0
          ? Math.round((data.failedBatches / data.totalBatches) * 100)
          : 0,
      productionEfficiency:
        data.totalPlanned > 0
          ? Math.round((data.totalProduced / data.totalPlanned) * 100)
          : 0,
      averageDurationMinutes:
        data.completedBatches > 0
          ? Math.round(data.totalDurationMinutes / data.completedBatches)
          : 0,
    ...
    return {
      ...
      summary: {
        ...
        highestEfficiencyWorkflow: workflowMetrics.reduce(
          (best, current) =>
            current.productionEfficiency > (best?.productionEfficiency || 0)
              ? current
              : best,
          null
        )?.workflowId,
    ...
    if (batches.length > 0) {
      ...
      if (utilizationScore < 60) {
        recommendations.push({
          ...
          description:
            'Production capacity may be underutilized. Consider increasing batch sizes or frequency.',
          impact: 'medium',
          effort: 'low',
        })
      }
  ...
  async calculateOverallUtilization(schedules, batches) {
    return { score: 75, details: {} }
  }
  async calculateStaffUtilization(schedules, batches) {
    return { average: 80, peak: 95, low: 60 }
  }
  async calculateEquipmentUtilization(schedules, batches) {
    return { average: 70, peak: 90, low: 50 }
  }
  async calculateTimeUtilization(schedules, batches) {
    return { efficiency: 85, waste: 15 }
  }
  async calculateUtilizationTrends(schedules, batches) {
    return { trend: 'improving', change: 5 }
  }
  async identifyUtilizationBottlenecks(schedules, batches) {
    return [{ type: 'staff', severity: 'medium' }]
  }
  async getHistoricalProductionData(days) {
    return { days, batches: [], trends: {} }
  }
  async calculateBaselineMetrics(historicalData) {
    return { volume: 100, efficiency: 80 }
  }
  async forecastProductionVolume(baseline, period) {
    return { predicted: baseline.volume * 1.1, range: [95, 115] }
  }
  async forecastEfficiency(baseline, period) {
    return { predicted: baseline.efficiency * 1.05, range: [75, 85] }
  }
  async forecastCapacityNeeds(baseline, period) {
    return { staffNeeds: 5, equipmentNeeds: ['oven'] }
  }
  async forecastQualityMetrics(baseline, period) {
    return { predicted: 95, risks: ['complexity'] }
  }
  async identifyForecastRisks(baseline, period) {
    return [{ risk: 'capacity', probability: 0.3 }]
  }
  async calculateConfidenceIntervals(forecast, level) {
    return { lower: 0.8, upper: 1.2 }
  }
  async calculateQualityOverview(batches) {
    return { score: 90, checks: 100, issues: 5 }
  }
  async calculateQualityTrends(batches) {
    return { trend: 'stable', change: 0 }
  }
  async analyzeQualityIssues(batches) {
    return { types: [], frequency: {} }
  }
  async identifyQualityImprovements(batches) {
    return [{ area: 'timing', impact: 'medium' }]
  }
  async calculateQualityCompliance(batches) {
    return { rate: 95, standards: ['ISO'] }
  }
  async calculateQualityCosts(batches) {
    return { total: 1000, savings: 200 }
  }
  async calculateEfficiencyBreakdown(metrics) {
    return { byWorkflow: {}, byStep: {} }
  }
  async compareToBenchmarks(metrics) {
    return { industry: 80, internal: 85 }
  }
  async generateEfficiencyImprovements(metrics) {
    return [{ area: 'scheduling', potential: 10 }]
  }
}
