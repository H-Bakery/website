  ...
  const ProductionSchedule = sequelize.define(
    'ProductionSchedule',
    {
      ...
      equipmentSchedule: {
        ...
        defaultValue: {},
        comment:
          'Equipment booking schedule {equipment: [{start, end, batchId}]}',
      },
    ...
    },
    {
      ...
      getterMethods: {
        ...
        staffUtilization() {
          ...
          return Math.round(
            ((this.totalStaffHours * 60) / plannedMinutes) * 100
          )
        },
        ...
        completionPercentage() {
          ...
          return Math.round(
            (this.completedBatches.length / this.plannedBatches.length) * 100
          )
        },
        ...
        isOverrun() {
          ...
          const endTime = new Date(
            `${this.scheduleDate}T${this.workdayEndTime}`
          )
        ...
        capacityUtilization() {
          ...
          return Math.round(
            (this.estimatedProductionTime / totalCapacityMinutes) * 100
          )
        },

        // Get active batches
        activeBatches() {
          if (!this.plannedBatches || !this.completedBatches)
            return this.plannedBatches || []

          return this.plannedBatches.filter(
            (batchId) => !this.completedBatches.includes(batchId)
          )
        },
        ...
        isFuture() {
          return (
            new Date(this.scheduleDate) > new Date().setHours(23, 59, 59, 999)
          )
        },
      },
    }
