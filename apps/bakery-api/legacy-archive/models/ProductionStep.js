    ...
    },
    {
      ...
      getterMethods: {
        ...
        delayMinutes() {
          ...
          return Math.round(
            (new Date() - new Date(this.plannedEndTime)) / (1000 * 60)
          )
        },
        ...
        needsAttention() {
          return (
            this.hasIssues ||
            this.isOverdue ||
            (this.qualityCheckRequired &&
              !this.qualityCheckCompleted &&
              this.status === 'completed')
          )
        },
        ...
        nextActivity() {
          ...
          return this.activities.find(
            (activity) => !this.completedActivities.includes(activity)
          )
        },
        ...
        isReadyToStart() {
          return (
            this.status === 'ready' ||
            (this.status === 'pending' && this.plannedStartTime <= new Date())
          )
        },
