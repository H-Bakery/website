  ...
  try {
    if (userId) {
      ...
      if (
        user &&
        user.email &&
        (await emailService.shouldSendEmail(userId, notification))
      ) {
...
async function createLowInventoryNotification(
  item,
  currentStock,
  minStock,
  unit = 'Stück'
    ...
    switch (type) {
      case 'sick_leave':
        ...
        variables = {
          ...
          date: staffData.date,
          coverageInfo:
            staffData.coverageInfo || 'Vertretung wird noch organisiert',
        }
        break
      case 'shift_change':
...
async function createNotificationFromTemplate(
  ...
  try {
    ...
    logger.error(
      `Error creating notification from template ${templateKey}:`,
      error
    )
    throw error
  }
