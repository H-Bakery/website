...
router.put('/policies', requireAdmin, async (req, res) => {
  try {
    ...
    if (
      typeof permanentDeleteAfterDays === 'number' &&
    ...
    if (updates.autoArchiveAfterDays && updates.permanentDeleteAfterDays) {
      if (updates.permanentDeleteAfterDays <= updates.autoArchiveAfterDays) {
        return res.status(400).json({
          success: false,
          error:
            'Permanent delete period must be longer than auto-archive period',
        })
      }
    }
  ...
  try {
    ...
    res.json({
      ...
      message: result.skipped
        ...
        : `Successfully archived ${result.archived} notifications`,
      result,
    })
  } catch (error) {
...
router.post('/cleanup', requireAdmin, async (req, res) => {
  try {
    ...
    res.json({
      ...
      message: result.skipped
        ? 'Cleanup is disabled'
        : `Successfully deleted ${result.deleted} notifications`,
      result,
    })
  } catch (error) {
  ...
  try {
    ...
    if (!policies.enabled) {
      return res.status(400).json({
        success: false,
        error:
          'Archival service is disabled. Enable it first by updating policies.',
      })
    }
