...
const recipeCreationRules = () => [
  ...
  body('yield')
    ...
    .trim()
    .isLength({ max: 50 })
    .withMessage('Yield must not exceed 50 characters'),
  ...
  body('ingredients.*')
    ...
    .isLength({ max: 200 })
    .withMessage('Each ingredient must not exceed 200 characters'),
  ...
  body('instructions.*')
    ...
    .isLength({ max: 1000 })
    .withMessage('Each instruction must not exceed 1000 characters'),
...
const recipeUpdateRules = () => [
  param('slug').trim().notEmpty().withMessage('Recipe slug is required'),
  ...
  body('ingredients')
    ...
    .isArray({ min: 1 })
    .withMessage(
      'Recipe must have at least one ingredient if updating ingredients'
    ),
  ...
  body('instructions')
    ...
    .isArray({ min: 1 })
    .withMessage(
      'Recipe must have at least one instruction if updating instructions'
    ),
