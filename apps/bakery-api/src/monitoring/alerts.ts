...
class AlertManager {
  ...
  private async checkRule(rule: AlertRule) {
    try {
      ...
      if (shouldAlert) {
        const now = new Date();
        const cooldownExpired = !state.lastFired || 
  ...
  startMonitoring(intervalMinutes: number = 1) {
    ...
    logger.info(`Alert monitoring started, checking every ${intervalMinutes} minutes`);
  }
}
...
alertManager.addRule({
  ...
  condition: async () => {
    ...
    try {
      const { sequelize } = await import('@bakery/api/core');
