...
exports.deleteBatch = async (req, res) => {
  try {
    ...
    const scheduleDate = new Date(batch.plannedStartTime).toISOString().split('T')[0];
    socketService.emitScheduleUpdate(scheduleDate, {
      type: 'batch_deleted',
      batchId: id
    });
...
exports.getProductionStatus = async (req, res) => {
  try {
    ...
    const pendingBatches = batches.filter(b => b.status === 'planned' || b.status === 'ready');
    const waitingBatches = batches.filter(b => b.status === 'waiting');
    const completedBatches = batches.filter(b => b.status === 'completed');

    // Calculate overview stats
    const totalBatches = batches.length;
    const totalQuantity = batches.reduce((sum, b) => sum + (b.actualQuantity || b.plannedQuantity), 0);
    
    // Calculate efficiency
    const completedOnTime = completedBatches.filter(b => 
      b.actualEndTime && b.plannedEndTime && 
    ...
    batches.forEach(batch => {
      // Batch events
      if (batch.actualStartTime && new Date(batch.actualStartTime) >= oneHourAgo) {
        timeline.push({
          type: 'batch_started',
          batchId: batch.id,
          batchName: batch.name,
      ...
      batch.ProductionSteps?.forEach(step => {
        ...
        if (step.qualityCheckTime && new Date(step.qualityCheckTime) >= oneHourAgo) {
          timeline.push({
            type: 'quality_check',
            batchId: batch.id,
            batchName: batch.name,
