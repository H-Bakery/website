...
class SocketService {
  ...
  emitBatchUpdate(batchId, update) {
    if (this.io) {
      this.io.to(`production-batch-${batchId}`).emit('production:batch:update', {
        batchId,
        ...update
      });
  ...
  emitScheduleUpdate(date, update) {
    if (this.io) {
      this.io.to(`production-schedule-${date}`).emit('production:schedule:update', {
        date,
        ...update
      });
  ...
  emitIssueReported(batchId, issue) {
    if (this.io) {
      this.io.to(`production-batch-${batchId}`).emit('production:issue:reported', {
        batchId,
        issue
      });
  ...
  emitQualityCheck(batchId, stepId, qualityData) {
    if (this.io) {
      this.io.to(`production-batch-${batchId}`).emit('production:quality:check', {
        batchId,
        stepId,
        qualityData
      });
